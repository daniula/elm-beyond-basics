...
isValid : Model -> Bool
isValid { nameError, locationError, ageError, bibError } =
    nameError
        == Nothing
        && locationError
        == Nothing
...
validate =
    validateName >> validateLocation >> validateAge >> validateBib
...
validateAge model =
    ...
    in
        if ageInt <= 0 then
            { model | ageError = Just "Age must be a possitive number" }
        else
...
validateBib model =
    ...
    in
        if bibInt <= 0 then
            { model | bibError = Just "Bib must be a possitive number" }
        else
